...
class MainTest(unittest.TestCase):
    ...
    def test_get_changing_value(self):
        ...
        up, down, right, left = tetris_formal.get_changing_value(
            copy.deepcopy(self.moving_s_block)
        )
    ...
    def test_check_boundary(self):
        self.assertTrue(
            tetris_formal.check_boundary(copy.deepcopy(self.moving_s_block))
        )
    ...
    def test_Rotate(self):
        ...
        filtered_item = [
            item
            for item in clockwise_s_block
            if item in [[40, 160], [80, 160], [40, 200], [0, 200]]
        ]
    ...
    def test_check_left_or_right(self):
        self.assertEqual(
            tetris_formal.move_left_or_right(
                "left", copy.deepcopy(self.moving_s_block)
            ),
            [
                [(70, 90, 120), 0, 160],
                [(70, 90, 120), 40, 160],
        ...
        self.assertNotEqual(
            tetris_formal.move_left_or_right("right", self.out_of_bound_block),
            [
                [(70, 90, 120), 80, 160],
                [(70, 90, 120), 120, 160],
                [(70, 90, 120), 80, 120],
    ...
    def test_remove_and_drop(self):
        ...
        tetris_formal.block_list = [[(40, 30, 20), i, 760] for i in range(0, 400, 40)]
        tetris_formal.block_list.extend(
            [[(40, 30, 20), i, 720] for i in range(0, 360, 40)]
        )
        tetris_formal.score = 0

        expect_block_list = []
        expect_block_list.extend([[(40, 30, 20), i, 760] for i in range(0, 360, 40)])
    ...
    def test_move_left_or_right(self, mock_check_left_or_right):
        ...
        # 測試右移
        moved_s_block = tetris_formal.move_left_or_right(
            "right", copy.deepcopy(self.moving_s_block)
        )
